#!/usr/bin/env python3
"""Verify that a loopback Cortex endpoint denies bad credentials before ingress."""
...
def main() -> int:
    if len(sys.argv) != 2:
        ...
        return 2
    token = os.environ.get("CORTEX_MCP_TOKEN", "")
    if not token:
        print("CORTEX_MCP_TOKEN is not set", file=sys.stderr)
        return 2
    wrong = "invalid-" + token[:8]
    try:
        observed = (status(sys.argv[1], None), status(sys.argv[1], wrong), status(sys.argv[1], token))
    except (OSError, URLError) as exc:
        print(f"Cortex authentication probe failed: {exc}", file=sys.stderr)
        return 1
