...
# Narrow compatibility guard: a pre-rename installation owns the same loopback
# port and contains credentials/state that must be migrated deliberately. Never
# guess or overwrite it during a normal install.
LEGACY_DEPLOYMENT=0
for legacy_path in \
  /etc/corthex/corthex.env \
  /opt/corthex/.venv \
  /var/lib/corthex \
  /etc/systemd/system/corthex-remote.service; do
  [[ -e "$legacy_path" || -L "$legacy_path" ]] && LEGACY_DEPLOYMENT=1
done
systemctl is-active --quiet corthex-remote.service && LEGACY_DEPLOYMENT=1 || true
if [[ $LEGACY_DEPLOYMENT -eq 1 && ${CORTEX_CONFIRMED_RENAME_UPGRADE:-0} != 1 ]]; then
  echo "Legacy Corthex deployment detected; refusing to mutate it." >&2
  echo "Follow docs/RENAME_UPGRADE.md, then rerun with CORTEX_CONFIRMED_RENAME_UPGRADE=1." >&2
  exit 3
fi
...
BANKS_JSON=${CORTEX_BANKS_JSON:-${existing_banks:-'{"cortex-shared":"Cortex shared memory"}'}}
