...
Cortex uses [Hindsight](https://github.com/vectorize-io/hindsight) as its underlying memory engine. Hindsight organizes memory into banks and provides the `retain`, `recall`, and `reflect` operations.[1] Cortex supplies the client integration, access boundary, policy, migration, and operations around it.

<p align="center">
  <img src="assets/cortex-memory-flow.svg" width="100%" alt="AI clients connect through Cortex to a shared Hindsight memory bank">
</p>

## Key features

- **One memory across tools.** Claude, Codex, ChatGPT, and other MCP clients can work from the same durable context.
- **Selective by design.** Clients retain useful facts and decisions rather than silently recording complete conversations.
- **A stable product boundary.** Cortex owns the CLI, MCP tools, bank policy, and result contracts while preserving clear Hindsight attribution.
- **Private remote access.** The Remote Brain deployment exposes only the authenticated Cortex facade over a tailnet; Hindsight and PostgreSQL remain private.
- **Portable operations.** A cross-platform CLI, deterministic migration, backup, restore, and rollback paths keep memory inspectable and recoverable.
...
<p align="center">
  <img src="assets/cortex-trust-boundary.svg" width="100%" alt="Cortex Remote Brain keeps Hindsight and PostgreSQL inside a private server boundary">
</p>
...
The migration toolkit provides a fail-closed path to a shared `cortex-shared` bank. It normalizes and deduplicates records, preserves source provenance, requires verified backup manifests before applying a plan, and leaves the legacy Windows `cortex` bank and other source banks intact. Read [Unified Cortex memory architecture](docs/UNIFIED_MEMORY.md) before using it.
...
- [`docs/RENAME_UPGRADE.md`](docs/RENAME_UPGRADE.md): safely migrate a pre-rename deployment with rollback
