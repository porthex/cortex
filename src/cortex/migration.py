...
AUTHORITATIVE_BANK = "cortex-shared"
# Compatibility is intentionally limited to state written by the immediately
# preceding product revision. It may be removed only after deployed upgrades
# and their rollback windows are complete.
LEGACY_PRODUCT_BANK = "corthex"
ROLLBACK_BANKS = {"hermes", LEGACY_PRODUCT_BANK}


def require_authoritative_bank(bank: str) -> None:
    """Prevent migration/finalization from ever targeting a legacy source."""
    if bank != AUTHORITATIVE_BANK:
        raise CortexError(f"mutating commands may target only the authoritative {AUTHORITATIVE_BANK} bank")
...
def update_hermes_config(path: Path) -> Path:
    ...
    Only the known ``hermes`` source, the immediately preceding product bank,
    and an idempotent ``cortex-shared`` rerun are accepted. An adjacent
    byte-preserving backup is always created.
    """
    try:
    ...
    if current.get("bank_id") not in {*ROLLBACK_BANKS, AUTHORITATIVE_BANK}:
        raise CortexError("unexpected Hermes source bank; refusing to reconfigure")
    if current.get("bank_id") == AUTHORITATIVE_BANK:
        valid = []
        for candidate in path.parent.glob(f"{path.name}.pre-cortex-shared-*.backup"):
            ...
            try:
                ...
                continue
            if candidate.name == f"{path.name}.pre-cortex-shared-{digest}.backup" and bank in ROLLBACK_BANKS:
    ...
    backup = path.with_name(f"{path.name}.pre-cortex-shared-{digest}.backup")
...
def rollback_hermes_config(path: Path, backup: Path) -> None:
    ...
    try:
        ...
        raise CortexError(f"cannot validate current Hermes configuration before rollback: {exc}") from exc
    if current.get("bank_id") != AUTHORITATIVE_BANK:
        raise CortexError("rollback requires the current authoritative Cortex configuration")
    try:
    ...
    if backup.name != f"{path.name}.pre-cortex-shared-{digest}.backup":
        raise CortexError("Hermes rollback backup digest does not match its filename")
    if restored.get("bank_id") not in ROLLBACK_BANKS:
        raise CortexError("rollback backup is not a recognized pre-upgrade configuration")
...
class HindsightAPI:
    ...
    def ensure_cortex_bank(self, bank: str = AUTHORITATIVE_BANK) -> Any:
    ...
    def configure_operational(self, bank: str = AUTHORITATIVE_BANK) -> Any:
        require_authoritative_bank(bank)
...
def _main(argv: list[str] | None = None) -> int:
    ...
    apply.add_argument("--bank", default=AUTHORITATIVE_BANK)
    apply.add_argument("--workers", type=int, default=4)
    apply.add_argument("--confirm", action="store_true")
    finalize = sub.add_parser("finalize")
    finalize.add_argument("--bank", default=AUTHORITATIVE_BANK)
    ...
    if args.command == "inventory":
        ...
        print(json.dumps({"config": str(args.config), "bank": AUTHORITATIVE_BANK, "backup": str(backup)}))
